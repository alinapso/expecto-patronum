...
function TableExampleSortable({ tableData }: any) {
	...
	const tableRows = data?.map((row: any) => {
		...
		if (row.patron) {
			patron_section = (
				<>
					<Table.Cell>{row.patron.firstName + " " + row.patron.lastName}</Table.Cell>
		...
		return (
			<Table.Row key={row.id}>
				<Table.Cell>{row.firstName + " " + row.middleName}</Table.Cell>
	...
	return (
		<Table sortable celled fixed>
			<Table.Header>
				<Table.Row>
					<Table.HeaderCell
						sorted={column === "firstName" ? direction : "descending"}
						onClick={() =>
							dispatch({
								type: "CHANGE_SORT",
								value: "firstName",
